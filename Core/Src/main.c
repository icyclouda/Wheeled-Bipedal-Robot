...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_TIM_Base_Start_IT(&htim3); // Timer3中断，不断发送访问寄存器数据帧
  // 设置控制板的xt30 5V控制口
	power1(1);
	power2(1);
	...
	bsp_fdcan_set_baud(&hfdcan1, CAN_CLASS, CAN_BR_1M); // can1的fdcan配置
	bsp_fdcan_set_baud(&hfdcan2, CAN_CLASS, CAN_BR_1M); // can2的fdcan配置
